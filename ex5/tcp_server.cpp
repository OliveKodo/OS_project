...
{
    // Parse request like "-e 5 -v 4 -s 43"
    ...
    try
    {
        ...
        // Check if it has Euler circuit
        if (graph.hasEulerCircuit())
        {
            std::vector<int> circuit = graph.findEulerCircuit();
            std::string result = "SUCCESS: Graph has Euler circuit!\n";
        ...
        {
            std::string result = "RESULT: Graph does NOT have Euler circuit\n";
            result += "Vertices: " + std::to_string(vertices) + "\n";
            result += "Edges: " + std::to_string(edges) + "\n";
